...
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import {
  faTornado, faShieldHalved, faWater, faGavel, faBomb, faUser
} from "@fortawesome/free-solid-svg-icons";
...
export default function NavBar() {
  ...
  // --- Nav links
  const extraNav = (logStatus === 1 || logStatus === 2) && (
    <>
      <a href="/otherprofiles" className="flex items-center gap-1 hover:text-[#FF6B6B]">
        <FontAwesomeIcon icon={faUser} />
        Other Profiles
      </a>
      <a href="/createlisting" className="flex items-center gap-1 hover:text-[#FF6B6B]">
        <FontAwesomeIcon icon={faBomb} />
        Create Listing
      </a>
      <a href="/profile" className="flex items-center gap-1 hover:text-[#FF6B6B]">
  ...
  const adminPanel =
    logStatus === 2 && (
      <a href="/admin" className="flex items-center gap-1 hover:text-[#FF6B6B] font-bold">
        <FontAwesomeIcon icon={faUser} />
        Admin Panel
      </a>
    );
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-10 bg-[#FFE9C6] text-[#3D405B] text-lg px-10 py-6 font-semibold items-center">
      {/* Nav Items & Extras */}
      <div className="col-span-9 flex flex-wrap gap-6 items-center">
        {navItems.map((item, idx) => (
          <a key={idx} href={item.href} className="flex items-center gap-1 hover:text-[#FF6B6B]">
            <FontAwesomeIcon icon={item.icon} />
            {item.name}
      ...
      {/* Login/Logout on far right, always in its own cell */}
